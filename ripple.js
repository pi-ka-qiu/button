...
var Ripple = function () {
    ...
    _createClass(Ripple, [{
        ...
        key: 'buttonAddClickEvent',

        //为所有按钮添加监听，添加span
        value: function buttonAddClickEvent() {
            var _this = this;
            ...
            var _loop = function _loop(i) {
                ...
                var wrapper = _this.createRippleWrapper(_this.CIRCULAR);
                ...
                //添加点击监听，创建span
                    ...
                    _this.reppleClick(event, _this.buttonList[i], _this.CIRCULAR, _this.createRippleChildNode(wrapper));
                }
                //添加鼠标移出 监听。设置span为可移除状态
                );_this.buttonList[i].addEventListener('mouseleave', function (event) {
                    _this.mouseFLAG = true;
                    _this.removeRipple(wrapper);
                });
            };
        ...
        key: 'createRippleWrapper',
        value: function createRippleWrapper(circular) {
            var _this3 = this;
        ...
        key: 'createRippleChildNode',
        value: function createRippleChildNode(wrapper) {
            var _this4 = this;
        ...
        key: 'freeElementCount',

        /**
         * 
         * @param {Element} wrapper 
         */
        value: function freeElementCount(wrapper) {
            var count = 0;
            var childrenList = wrapper.children;
            for (var i = 0; i < childrenList.length; i++) {
                var flag = ripple_utill.hasClass(childrenList[i], this.CIRCULAR.animationCName);
         ...
         */
        value: function reppleClick(event, button, circular, ripple) {
         ...
         * @param {Button} button 
         */
        value: function computedCircleCenter(event, button) {
            var positionX = 0;
            var positionY = 0;
         ...
         */
        value: function removeRipple(wrapper) {
            //如果可以移除span
            if (this.mouseFLAG && this.animateFLAG) {
                var count = this.freeElementCount(wrapper);
